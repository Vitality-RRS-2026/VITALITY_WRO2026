...
/* --- corridor margin ----------------------------------------------------
   The robot no longer tries to sit centred between the walls. It only
   steers when it comes within WALL_MARGIN_CM of a wall; anywhere in the
   middle it just holds its heading and drives straight.

   WHY: centring means the controller is always correcting, and every
   correction leaves the robot slightly angled. A robot at an angle gives
   bad ultrasonic readings, because the pulse strikes the wall off-square
   and reads long or not at all - which is what produced the false corners.
   Driving straight through the middle of the corridor removes most of
   that.

   The correction is proportional to how far INSIDE the margin the robot
   has come, so it is zero at exactly 15 cm and grows smoothly as the wall
   gets closer. That avoids the jerk of a hard on/off rule.
   ------------------------------------------------------------------------ */
...
const float TURN_TOLERANCE_DEG = 20.0f;
